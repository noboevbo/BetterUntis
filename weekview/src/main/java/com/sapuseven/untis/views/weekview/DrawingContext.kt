...
class DrawingContext(val startPixel: Float) {
	...
	companion object {
		internal fun create(config: WeekViewConfig): DrawingContext {
			...
			val offset =
				if (config.snapToWeek) DateUtils.offsetInWeek(today, config.firstDayOfWeek) else 0
			val offsetCompensation = DateUtils.offsetInWeek(today, config.firstDayOfWeek) - offset
			dayRange.addAll(
				DateUtils.getDateRange(
					today.plusDays(
						DateUtils.actualDays(
							daysScrolled + offsetCompensation,
							config.weekLength
						) - offset - offsetCompensation
					), config.visibleDays + 1, config.firstDayOfWeek, config.weekLength
				)
			)
