...
class UntisErrorTest {
	...
	fun untisError_serialization() {
		assertThat(
			getJSON().encodeToString<UntisError>(
				UntisError(
					code = 1000,
	...
	fun untisError_deserialization() {
		val error1 =
			getJSON().decodeFromString<UntisError>("""{"code":1000,"message":"error message"}""")
		...
		val error2 =
			getJSON().decodeFromString<UntisError>("""{"code":1000,"message":"error message","data":{"serverTime":123456}}""")
