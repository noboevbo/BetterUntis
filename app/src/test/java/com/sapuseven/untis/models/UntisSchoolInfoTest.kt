...
class UntisSchoolInfoTest {
	...
	fun untisSchoolInfo_serialization() {
		assertThat(
			getJSON().encodeToString<UntisSchoolInfo>(
				UntisSchoolInfo(
					server = "server",
	...
	fun untisSchoolInfo_deserialization() {
		val schoolInfo =
			getJSON().decodeFromString<UntisSchoolInfo>("""{"server":"server","useMobileServiceUrlAndroid":true,"useMobileServiceUrlIos":false,"address":"123","displayName":"school display name","loginName":"LOGIN_NAME","schoolId":123,"serverUrl":"http://","mobileServiceUrl":"http://"}""")
