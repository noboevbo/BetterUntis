...
class ParamsTest {
	...
	fun appSharedSecretParams_serialization() {
		assertThat(
			getJSON().encodeToString<AppSharedSecretParams>(
	...
	fun schoolSearchParams_serialization() {
		assertThat(
			getJSON().encodeToString<SchoolSearchParams>(
				SchoolSearchParams(
					search = "school",
					schoolid = 123,
					schoolname = "name"
	...
	fun userDataParams_serialization() {
		assertThat(
			getJSON().encodeToString<UserDataParams>(
				UserDataParams(
					auth = UntisAuth(
						user = "user",
						otp = 123456L,
	...
	fun appSharedSecretParams_deserialization() {
		val appSharedSecretParams =
			getJSON().decodeFromString<AppSharedSecretParams>("""{"userName":"user","password":"pass"}""")
	...
	fun schoolSearchParams_deserialization() {
		val schoolSearchParams =
			getJSON().decodeFromString<SchoolSearchParams>("""{"search":"school","schoolid":123}""")
	...
	fun userDataParams_deserialization() {
		val userDataParams =
			getJSON().decodeFromString<UserDataParams>("""{"auth":{"user":"user","otp":123456,"clientTime":123456}}""")
